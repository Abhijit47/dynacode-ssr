...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    function handleDropdownClick(e: Event) {
      ...
      // Ensure currentTarget is an Element before accessing DOM properties
      if (!(btn instanceof Element)) return;

      const dropdownMenu = btn.parentElement
        ?.nextElementSibling as Element | null;
      if (!dropdownMenu) return;

      const isOpen = dropdownMenu.classList.contains('active');

      const menus =
        sidebarRef.current?.querySelectorAll('.sidebar-dropdown-menu') ?? [];
      menus.forEach((menu) => {
        ...
        <ul className='menu'>
          ...
          <li className='sidebar-dropdown'>
            <div className='dropdown-header'>
              <a href='#'>Pages</a>
              <button className='sidebar-dropdown-btn'>
                <span className={'visually-hidden'}>Toggle Pages Dropdown</span>
                <i className='fa-solid fa-angle-down'></i>
              </button>
            </div>
            <ul className='sidebar-dropdown-menu'>
